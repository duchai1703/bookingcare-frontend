// src/containers/System/Admin/Specialty/EditSpecialtyModal.jsx
...
const EditSpecialtyModal = ({ isOpen, onClose, specialty, onSuccess }) => {
  ...
  useEffect(() => {
    if (!isOpen || !specialty) return;

    let parsedPhotos = [];
    if (Array.isArray(specialty.photos)) {
      parsedPhotos = specialty.photos;
    } else if (typeof specialty.photos === 'string') {
      try {
        parsedPhotos = JSON.parse(specialty.photos);
      } catch {
        parsedPhotos = [];
      }
    }

    const prevImg = specialty.image
      ? (specialty.image.startsWith('http') ? specialty.image : CommonUtils.decodeBase64Image(specialty.image))
      : '';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await editSpecialty(payload);
      if (res && res.errCode === 0) {
        toast.success(`Cập nhật chuyên khoa "${formData.name}" thành công!`);
        if (onSuccess) onSuccess();
        onClose();
      } else {
        setErrorMsg(res?.message || res?.errMessage || 'Không thể cập nhật chuyên khoa');
      }
    } catch (err) {
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <h3 style={{ margin: 0, fontSize: '1.08rem', fontWeight: 800, color: '#0F172A' }}>
                Chỉnh sửa Hồ sơ Chuyên khoa #{specialty.id}
              </h3>
              <span style={{ fontSize: '0.78rem', color: '#64748B' }}>
                Cập nhật thông tin định danh, chỉ tiêu tiếp nhận, slider ảnh và tài liệu giới thiệu
              </span>
            </div>
          </div>
          <button
          ...
          <div style={{ marginBottom: 20, padding: 14, background: '#F8FAFC', borderRadius: 10, border: '1px solid #E2E8F0' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
              <ImageUploadInput
                ...
                shape="rect"
                inputId="specialty-avatar-edit-upload"
              ...
              <div style={{ fontSize: '0.74rem', color: '#64748B', lineHeight: 1.5 }}>
                Đổi ảnh đại diện chuyên khoa nếu cần cập nhật giao diện hiển thị.<br />
